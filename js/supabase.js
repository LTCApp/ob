...
class SupabaseClient {
    ...
    async getAds({ category = '', neighborhood = '', search = '', page = 1, limit = APP_CONFIG.adsPerPage, status = '' } = {}) {
        ...
        // Order: featured first, then by created_at desc
        // Using computed status ordering: featured=0, active=1
        return this.request(`ads?select=*&${filters.join('&')}&order=status.asc,created_at.desc&offset=${from}&limit=${limit}`);
    }
...
console.log('Supabase client loaded ✓ (SDK mode, with dynamic data support)');