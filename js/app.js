...
// ===== User Notification System (localStorage-based) =====
function getUserNotifications(userId) {
    if (!userId) return [];
    try {
        return JSON.parse(localStorage.getItem(`ob_notifs_${userId}`) || '[]');
    } catch(e) { return []; }
}
...
function markAllNotificationsRead(userId) {
    if (!userId) return;
    try {
        const notifs = getUserNotifications(userId).map(n => ({ ...n, is_read: true }));
        localStorage.setItem(`ob_notifs_${userId}`, JSON.stringify(notifs));
    } catch(e) {}
}
...
function initNotificationBell(userId) {
    const count = getUnreadNotifCount(userId);
