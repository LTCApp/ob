...
function createAdCard(ad) {
    ...
    return `
        <article class="ad-card${isFeatured ? ' featured' : ''}" data-id="${ad.id}">
            <a href="ad-detail.html?id=${ad.id}" class="ad-card-link">
                <div class="ad-card-image">
                    ${placeholder}
                    ${isFeatured ? `<span class="featured-badge">⭐ مميز</span>` : ''}
                    ${isExpiring ? `<span class="ad-card-badge expiring">ينتهي خلال ${daysLeft} يوم</span>` : ''}
                </div>
                <div class="ad-card-content">
                    <h3 class="ad-card-title">${sanitize(ad.title)}</h3>
                    ...
                    <div class="ad-card-location">
                        ...
                        <span>${sanitize(ad.neighborhood || APP_CONFIG.location)}</span>
                    </div>
                </div>
            </a>
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (recentAdsContainer) {
        try {
            const ads = await supabaseDB.getAds({ limit: 8 });
...
async function getUserNotifications(userId) {
    if (!userId) return [];
    try {
        if (window.supabaseDB && window.supabaseDB.getUserNotifications) {
            return await window.supabaseDB.getUserNotifications(userId);
        }
        return [];
    } catch(e) { return []; }
}
...
async function markAllNotificationsRead(userId) {
    if (!userId) return;
    try {
        if (window.supabaseDB && window.supabaseDB.markUserNotificationsRead) {
            await window.supabaseDB.markUserNotificationsRead(userId);
        }
    } catch(e) {}
}
...
async function initNotificationBell(userId) {
    const count = await getUnreadNotifCount(userId);
...
(function initSmartHeader() {
    ...
    let lastScrollTop = 0;
    let scrollThreshold = 80; // Minimum scroll before hiding
    let isHidden = false;
