...
// App Configuration - will be loaded from Supabase (empty defaults)
let APP_CONFIG = {
    siteName: '',
    siteFullName: '',
    location: '',
...
function updateSiteBranding(settings = null) {
    ...
    if (config.logo) {
        // Specific handling for logoImage and footerLogoImage
        ...
        if (footerLogoImage) {
            footerLogoImage.src = config.logo;
            footerLogoImage.style.display = 'block';
        }
        if (footerLogoSvg) footerLogoSvg.style.display = 'none';
    }
    
    // Update hero title if exists
    const heroTitle = document.querySelector('.hero-content h1');
    if (heroTitle) {
        heroTitle.textContent = config.siteFullName || '';
    }
    ...
    if (footerBrandP && config.seo && config.seo.description) {
        footerBrandP.textContent = config.seo.description;
    }
    
    // Update footer copyright
    const footerCopyright = document.querySelector('.footer-bottom p');
    if (footerCopyright && config.siteName) {
        const year = new Date().getFullYear();
        footerCopyright.textContent = `© ${year} ${config.siteName} - جميع الحقوق محفوظة.`;
    }
    
    // Update admin sidebar logo text
    const adminLogoSpan = document.querySelector('.admin-sidebar-logo span');
    if (adminLogoSpan && config.siteName) {
        adminLogoSpan.textContent = config.siteName + ' Admin';
    }
}

// Initialize data on page load
async function initializeAppData() {
    // Show loading state for logos
    ...
    // Show logo text after loading
    if (logoText) logoText.style.visibility = 'visible';
    if (footerSiteName) footerSiteName.style.visibility = 'visible';
    
    // Show svg logo only if no custom logo
    if (APP_CONFIG.logo) {
        if (logoSvg) logoSvg.style.display = 'none';
        if (footerLogoSvg) footerLogoSvg.style.display = 'none';
    } else {
        if (logoSvg) logoSvg.style.display = 'block';
        if (footerLogoSvg) footerLogoSvg.style.display = 'block';
    }
