...
class SupabaseClient {
    ...
    async getAds({ category = '', neighborhood = '', search = '', page = 1, limit = APP_CONFIG.adsPerPage, status = '' } = {}) {
        ...
        const query = `ads?select=*&${filterStr}order=created_at.desc&offset=${from}&limit=${limit}`;
    ...
    async uploadImage(file, adId) {
        try {
            ...
            const response = await fetch(
                `${this.url}/storage/v1/object/ad-images/${filePath}`,
                {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`,
        ...
        for (const cat of CATEGORIES) {
            try {
                const result = await this.request(
                    `ads?category=eq.${cat.id}&expires_at=gte.${today}&select=id&status=in.(active,featured)`
                );
...
console.log('Supabase client loaded - createAd ready');
