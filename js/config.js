...
// WhatsApp link builder - Egypt country code +20
App.getWhatsAppLink = function(phone, message) {
    if (!phone) return '#';
    // Strip leading 0, prepend 20 → wa.me/201XXXXXXXXX
    const number = '20' + String(phone).replace(/^0+/, '').replace(/\D/g, '');
    return 'https://wa.me/' + number + (message ? '?text=' + encodeURIComponent(message) : '');
};
window.App = window.App || {};
Object.assign(window.App, { getWhatsAppLink: App.getWhatsAppLink });
