...
class SupabaseClient {
    ...
    async createAd(adData) {
        ...
        // Validate category exists in CATEGORIES
        if (data.category && CATEGORIES.length > 0) {
            const validCat = CATEGORIES.find(c => c.id === data.category);
            if (!validCat) {
                console.warn('Category not found in CATEGORIES, refreshing...');
                await loadCategories();
                const refreshedCat = CATEGORIES.find(c => c.id === data.category);
                if (!refreshedCat) {
                    throw new Error('القسم المحدد غير موجود. يرجى اختيار قسم صحيح من القائمة.');
                }
            }
        }
        ...
        if (this.sdk) {
            ...
            if (error) {
                ...
                if (error.message && error.message.includes('ads_category_check')) {
                    throw new Error('خطأ في القسم: يرجى التأكد من اختيار قسم صحيح من القائمة.');
                }
                if (error.message && error.message.includes('check constraint')) {
                    throw new Error('خطأ في البيانات المدخلة. يرجى مراجعة جميع الحقول.');
                }
                throw new Error(error.message || 'فشل في إنشاء الإعلان');
            }
            return result;
        }
